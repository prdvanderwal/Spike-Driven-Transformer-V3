...
class HARDVS(NeuromorphicDatasetFolder):
    ...
    def extract_downloaded_files(download_root: str, extract_root: str):
        ...
        extract_archive(
            os.path.join(download_root, "MINI_HARDVS_files.zip"), temp_ext_dir
        )
        ...
        shutil.copy(
            os.path.join(download_root, "test_label.txt"),
            os.path.join(extract_root, "test_label.txt"),
        )
        shutil.copy(
            os.path.join(download_root, "train_label.txt"),
            os.path.join(extract_root, "train_label.txt"),
        )
        shutil.copy(
            os.path.join(download_root, "val_label.txt"),
            os.path.join(extract_root, "val_label.txt"),
        )
                ...
                for line in txt_file:
                    if line.__len__() > 1:
                        ...
                        source_file = os.path.join(
                            extract_root, class_name, sample_name + ".npz"
                        )
                        # if os.path.exists(source_file):
                        target_file = os.path.join(
                            target_dir, class_name, sample_name + ".npz"
                        )
