...
def chunk_evs_pol_dvs_gesture(data, dt=1000, T=500, size=[2, 304, 240], ds=[4, 4]):
    try:
        t_start = data[0][0]
...
def chunk_evs_pol_dvs_gait(data, dt=1000, T=500, size=[2, 304, 240], ds=[4, 4]):
    ...
    for i, t in enumerate(ts):
        ...
        if idx_end > idx_start:
            ee = data[idx_start:idx_end, 1:]
            pol, x, y = ee[:, 2], (ee[:, 0] // ds[0]).astype(np.int64), (ee[:, 1] // ds[1]).astype(np.int64)
            np.add.at(chunks, (i, pol, x, y), 1)
        idx_start = idx_end
    return chunks

def chunk_evs_pol_NMNIST(data, dt=1000, T=500, size=[2, 304, 240], ds=[4, 4]):
    if len(data) == 0:
        return np.zeros([T] + [2] + [int(34 / ds[0])] + [int(34 / ds[1])], dtype='int64')
    t_start = data[0][0]
    ts = range(t_start, t_start + T * dt, dt)
    chunks = np.zeros([len(ts)] + [2] + [int(34 / ds[0])] + [int(34 / ds[1])], dtype='int64')
    idx_start = 0
    idx_end = 0
    for i, t in enumerate(ts):
        idx_end += find_first(data[idx_end:, 0], t + dt)
